...
const day_number_of_date = (month, day, year) => {
    if (month < 0 || day < 0 || year < 0){
        ...
        if (month == 1){
            ...
            // days += 28
            // days = is_leap_year(year) ? days + 1 : days
        ...
        if (is_leap_year(year)){  
            if (day == 29 || day == 1){   // Check if the day is the first or last day of the month
                if (day == 29){   // If it is the last day of February in a leap year, we do the following
                    ...
                    nex_day = 1
                    nex_year = year
                    // nex_date = new Date(`${nex_year}-${nex_month}-${nex_day}`)
                    nex_date = `${nex_year}-${nex_month}-${nex_day}`
                    
                    prev_month = month
                    prev_day = day - 1
                    prev_year = year
                    // prev_date = new Date(`${prev_year}-${prev_month}-${prev_day}`)
                ...
                // nex_date = new Date(`${year}-${month}-${day+1}`)
                nex_date = `${year}-${month}-${day+1}`
                // prev_date = new Date(`${year}-${month}-${day-1}`)
...
const onChange = e => {
    ...
    if(e.target.value != ''){
        // Split the selected date by "-" to get month, year, day
        let split_val = e.target.value.split('-')
        let year = Number(split_val[0])
        let month = Number(split_val[1])
        let day = Number(split_val[2])


        // This is our targeted output in our DOM
        ...
        if(is_valid_date(month, day, year)){
            ...
            output.innerHTML = `
                <div style="padding: 5px">
                    <div>The selected date is <span class="green-text1">${e.target.value}</span></div>
                    <div>It's ${calendarDay(month, day, year)} of ${e.target.value}</div>
            ...
            output.style.visibility = 'visible'
            output.innerHTML = `
                <div style="padding: 5px; color: red">Date entered is invalid!</div>
            `
        }
    }
}
